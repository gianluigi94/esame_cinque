...
function initializeCarousels(): void {
    const carousels = document.querySelectorAll<HTMLElement>('.carousel-wrapper');
    let marginTolerance: number = 11; // Margine di tolleranza in pixel per permettere un 'bel' scroll

    carousels.forEach((carousel) => { // ciclo su tutti i caroselli
        const track = carousel.querySelector<HTMLElement>('.image-track'); // contenitore padre
        const slider = carousel.querySelector<HTMLElement>('.image-slider'); // contenitore medio
        // HTMLElement è il tipo base per tutti gli elementi HTML in TypeScript.
        const images = track ? track.querySelectorAll<HTMLImageElement>('img') : []; // immagine all'interno del contenitore medio
        const totalImages = images.length; // lunghezza dell'immagine
        let position: number = 0;

        // bottoni all'interno del carosello
        const prevBtn = carousel.querySelector<HTMLElement>('.prev-btn');
        const nextBtn = carousel.querySelector<HTMLElement>('.next-btn');
         ...
         */
        function updateCarouselSettings(): void {
...
// per la card
// per la card
const card = document.getElementById("dynamic-card") as HTMLElement; // elemento di prova che simulerà le carte personalizzate per ogni cambio immagine/video per ogni film
const cardImage = document.getElementById("card-image") as HTMLImageElement; // immagine di default
const cardVideo = document.getElementById("card-video") as HTMLVideoElement; // video di default
const footer = document.querySelector("footer") as HTMLElement | null;

// larghezza minima della card
const minCardWidth: number = 200;
